...
# We can log in plaintext to stdout also, even though Admin uses
...
sleep 5 # give time for pmcd's startup messages, so it doesn't intermix with Admin's


exec bin/build-tool-detector ${1+"$@"}