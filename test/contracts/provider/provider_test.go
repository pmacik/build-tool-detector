...
// TestBuildToolDetectorAPIProvider verifies the provider
func TestBuildToolDetectorAPIProvider(t *testing.T) {

	var pactDir = os.Getenv("PACT_DIR")
	var pactProviderBaseURL = os.Getenv("PACT_PROVIDER_BASE_URL")
	var authURL = os.Getenv("PACT_AUTH_URL")
	...
	providerInfo := Setup(providerSetupHost, providerSetupPort, authURL, userName, userPassword)
